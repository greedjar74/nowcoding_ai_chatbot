...
def MZ_stage_4_while_notDone():
    ...
    # system content
    with open('system_contents/MZ_stage_4_while_notDone.txt', 'r', encoding='utf-8') as f:
        system_content = f.read()    
    st.sidebar.markdown("# System Content")
    st.sidebar.text(system_content)

    # teaching prompt
    with open('input_contents/MZ_stage_4_while_notDone.txt', 'r', encoding='utf-8') as f:
        teaching_prompt = f.read()    
    st.sidebar.markdown("# Teaching Prompt")
    st.sidebar.text(teaching_prompt)
    
    # Test case
    with open('test_cases/MZ_stage_4_while_notDone.txt', 'r', encoding='utf-8') as f:
        test_case = f.read()    
    st.sidebar.markdown("# Test Case")
    st.sidebar.text('# MZ05PY001V')
    st.sidebar.image('testcase_imgs/MZ_stage_4_while_notDone_testcase_img.PNG')
