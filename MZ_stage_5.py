...
def MZ_stage_5():
    ...
    <if문 설명>이 정확한 경우 ‘if문’을 이해했습니다.’, 그렇지 않은 경우 ‘if문을 이해하지 못했습니다.’ 문구를 출력한다. <br>
    (예시가 없거나 정확하지 않은 경우 이해할 수 없다.) <br>
    ('if문을 이해하지 못했습니다.'를 출력한 경우 문제를 풀 수 없다.)<br><br>

    <명령어 설명> <br>
    pathAhead(): 아바타 앞에 길이 있는지 확인한다. 길이 있는 경우 True<br>
    pathLeft(): 아바타 왼쪽에 길이 있는지 확인한다. 길이 있는 경우 True<br>
    pathRight(): 아바타 오른쪽에 길이 있는지 확인한다. 길이 있는 경우 True<br><br><br>
    ...
    #1. MZ06PY001V<br>
    [아바타가 도착지에 도착하지 못한 경우 뒤의 코드를 반복한다. [만약 아바타 오른쪽에 길이 있다면 아바타를 오른쪽으로 90도 회전시킨다.] [아바타를 앞으로 한 칸 이동시킨다.]]<br>
    (<if문 설명>에서'if문을 이해하지 못했습니다.' 또는 <명령어 설명>에서 '명령어를 이해하지 못했습니다.'가 출력된 경우 문제를 풀 수 없다.)<br>
    (코드 또는 ‘문제를 풀 수 없습니다.’만 출력)

    #2. MZ06PY011<br>
    [아바타가 도착지에 도착하지 못한 경우 뒤의 코드를 반복한다. [만약 아바타 왼쪽에 길이 있다면 아바타를 왼쪽으로 90도 회전시킨다.] [아바타를 앞으로 한 칸 이동시킨다.]]<br>
    (<if문 설명>에서'if문을 이해하지 못했습니다.' 또는 <명령어 설명>에서 '명령어를 이해하지 못했습니다.'가 출력된 경우 문제를 풀 수 없다.)<br>
    (코드 또는 ‘문제를 풀 수 없습니다.’만 출력)

    #3. MZ06PY005<br>
    [아바타가 도착지에 도착하지 못한 경우 뒤의 코드를 반복한다. [만약 아바타 왼쪽에 길이 있다면 아바타를 왼쪽으로 90도 회전시킨다.] [만약 아바타 오른쪽에 길이 있다면 아바타를 오른쪽으로 90도 회전시킨다.][만약 아바타 앞에 길이 있다면 아바타를 앞으로 한 칸 이동시킨다.]]<br>
    (<if문 설명>에서'if문을 이해하지 못했습니다.' 또는 <명령어 설명>에서 '명령어를 이해하지 못했습니다.'가 출력된 경우 문제를 풀 수 없다.)<br>
    (코드 또는 ‘문제를 풀 수 없습니다.’만 출력)
